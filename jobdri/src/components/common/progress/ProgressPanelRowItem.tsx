...
export default function ProgressPanelRowItem({
  ...
  return (
    <div
      className={clsx(
        "relative flex w-fit flex-row px-5 py-4 justify-end items-start gap-4 self-stretch rounded-card-result p-2",
        className,
        isInProgress
          ? "bg-icon-neutral-white rounded-card-s"
          : "bg-transparent",
      )}
    >
      <div
        className={clsx(
          "relative z-10 flex",
          // 상태에 따라 정렬 축을 다르게 설정합니다.
          isInProgress
            ? "items-center justify-center h-5 w-10 rounded-icon-round bg-fill-tertiary-default" // 중앙 정렬
            : "items-start justify-start h-5 w-10", // 나머지는 기존대로 좌상단 정렬
        )}
      >
        {isInProgress ? (
          <LoadMotion
            // py-1 제거 및 flex 추가로 확실한 중앙 정렬
          ...
          <span className="flex aspect-square h-5 w-5 items-start gap-2.5 rounded-icon-round bg-fill-secondary-default">
            <Icon
              type="CHECK_M"
      ...
      <div className="relative z-10 flex flex-col items-start gap-1">
        ...
        <span
          className={clsx(
            "w-[132px] max-w-[132px] text-left text-cap12-med tracking-normal [font-feature-settings:'liga'_off,'clig'_off]",
